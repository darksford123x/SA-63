...
type StatusQuery struct {
	...
	// eager-loading edges.
	withStatusInvoice *RepairInvoiceQuery
...
func (sq *StatusQuery) QueryStatusInvoice() *RepairInvoiceQuery {
...
func (sq *StatusQuery) WithStatusInvoice(opts ...func(*RepairInvoiceQuery)) *StatusQuery {
	query := &RepairInvoiceQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withStatusInvoice = query
	return sq
}
...
func (sq *StatusQuery) sqlAll(ctx context.Context) ([]*Status, error) {
	...
	if query := sq.withStatusInvoice; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Status, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(status.StatusInvoicePrimaryKey[0], fks...))
			},
		...
		query.Where(repairinvoice.IDIn(edgeids...))
