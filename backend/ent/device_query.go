...
type DeviceQuery struct {
	...
	// eager-loading edges.
	withDevices *RepairInvoiceQuery
...
func (dq *DeviceQuery) QueryDevices() *RepairInvoiceQuery {
...
//  WithDevices tells the query-builder to eager-loads the nodes that are connected to
// the "devices" edge. The optional arguments used to configure the query builder of the edge.
func (dq *DeviceQuery) WithDevices(opts ...func(*RepairInvoiceQuery)) *DeviceQuery {
	query := &RepairInvoiceQuery{config: dq.config}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context) ([]*Device, error) {
	...
	if query := dq.withDevices; query != nil {
		...
		query.Where(predicate.RepairInvoice(func(s *sql.Selector) {
			s.Where(sql.InValues(device.DevicesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.device_devices
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "device_devices" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "device_devices" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Devices = n
		}
	}
