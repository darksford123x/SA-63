...
// QueryStatusinvoice chains the current query on the statusinvoice edge.
func (riq *RepairInvoiceQuery) QueryStatusinvoice() *StatusQuery {
	query := &StatusQuery{config: riq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := riq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithStatusinvoice tells the query-builder to eager-loads the nodes that are connected to
// the "statusinvoice" edge. The optional arguments used to configure the query builder of the edge.
func (riq *RepairInvoiceQuery) WithStatusinvoice(opts ...func(*StatusQuery)) *RepairInvoiceQuery {
	query := &StatusQuery{config: riq.config}
	for _, opt := range opts {
		opt(query)
	}
	riq.withStatusinvoice = query
	return riq
}
...
//		GroupBy(repairinvoice.FieldRepairInvoiceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.RepairInvoice.Query().
//		Select(repairinvoice.FieldRepairInvoiceID).
//		Scan(ctx, &v)
//
...
func (riq *RepairInvoiceQuery) sqlAll(ctx context.Context) ([]*RepairInvoice, error) {
	var (
		...
		loadedTypes = [3]bool{
			riq.withDevice != nil,
			riq.withStatusinvoice != nil,
			riq.withSymptom != nil,
		}
	)
	if riq.withDevice != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := riq.withDevice; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].device_devices; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "device_devices" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := riq.withStatusinvoice; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*RepairInvoice, len(nodes))
		...
		var (
			edgeids []int
			edges   = make(map[int][]*RepairInvoice)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(repairinvoice.StatusinvoicePrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, riq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "statusinvoice": %v`, err)
		}
		query.Where(status.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "statusinvoice" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Statusinvoice = append(nodes[i].Edges.Statusinvoice, n)
			}
		}
	}
	...
	if query := riq.withSymptom; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(repairinvoice.SymptomPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, riq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "symptom": %v`, err)
		}
		query.Where(symptom.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "symptom" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Symptom = append(nodes[i].Edges.Symptom, n)
			}
		}
	}
